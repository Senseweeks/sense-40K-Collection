...
import type { CampaignControlState, CampaignEventRun, CampaignIdentity, ColdEmberLedger, EventTemplate, RegionalPressure, TravelRecord, TravelRoute, TypedProject, WorldLocation } from '../../types';
...
const playerAtlasDeckCss = `
  .atlas-command-deck{display:grid;grid-template-columns:minmax(190px,.72fr) minmax(0,2.45fr) minmax(175px,.7fr);grid-template-rows:auto minmax(0,1fr);gap:12px;max-width:1420px;padding:16px;align-items:stretch}
  .atlas-command-top{grid-column:2/4;display:flex;align-items:center;justify-content:space-between;gap:18px;padding:14px 18px;border:1px solid #806a3d;border-bottom:3px solid #bc9a50;background:linear-gradient(100deg,#241a15,#321d19 58%,#171312);box-shadow:inset 0 1px 0 rgba(255,224,157,.14),inset 0 -12px 30px rgba(0,0,0,.18)}
  ...
  .atlas-ongoings{grid-row:1/3;display:flex;flex-direction:column;gap:10px;padding:16px 14px;border:1px solid #76574a;border-left:3px solid #b88b55;background:linear-gradient(180deg,#251b17,#151715 38%);min-width:0}.atlas-ongoings h2,.atlas-quick h2{margin:0;color:#f1d8a2;font-size:1.05rem}.atlas-rail-note{margin:0;color:#b9c6bd;font-size:.86rem;line-height:1.45}
  ...
  @media(max-width:1050px){.atlas-command-deck{grid-template-columns:minmax(180px,.65fr) minmax(0,1.75fr)}.atlas-command-top{grid-column:2}.atlas-quick{grid-column:2;grid-row:3;display:grid;grid-template-columns:repeat(auto-fit,minmax(155px,1fr))}.atlas-quick h2,.atlas-quick .eyebrow{grid-column:1/-1}.atlas-ongoings{grid-row:1/4}.atlas-system-panel{grid-column:2}}
...
const inquisitorialBridgeCss = `
  .atlas-command-deck{position:relative;isolation:isolate;max-width:1480px;padding:22px;background:radial-gradient(ellipse at 50% -30%,rgba(114,44,24,.2),transparent 42%),linear-gradient(90deg,rgba(3,7,8,.97),rgba(13,17,16,.98) 28%,rgba(7,10,11,.98) 72%,rgba(3,6,7,.98));border:1px solid #493a28;box-shadow:0 20px 80px rgba(0,0,0,.58),inset 0 0 0 1px rgba(213,177,99,.08)}
  ...
  .atlas-bridge-title{position:relative;z-index:1;display:flex;align-items:center;gap:12px}.atlas-inquisition-sigil{display:grid;place-items:center;width:43px;height:52px;border:1px solid #b88d48;color:#e4c57d;background:linear-gradient(145deg,#351d16,#111312);font:700 2rem Georgia,serif;line-height:1;text-shadow:0 0 13px rgba(226,189,110,.42);box-shadow:inset 0 0 0 3px rgba(0,0,0,.32)}
  .atlas-command-top h1{letter-spacing:.035em}.atlas-command-top .eyebrow{color:#d1b36e}.atlas-tool-strip{position:relative;z-index:1}.atlas-tool-strip a{position:relative;border-color:#897342;background:linear-gradient(180deg,#47301f,#1a1714);box-shadow:inset 0 1px 0 rgba(255,239,188,.11)}
...
export function PlayerAtlas({ report }: {
    ...
    useEffect(() => { void request<Partial<WorldData>>('/campaign/world', { headers: { 'X-Trusted-Local-Role': 'player' } }).then(setData).catch(error => report(error.message)); }, []);
    const locations = data?.locations ?? [];
    const selected = locations.find(location => location.id === selectedId) ?? locations[0];
    const ongoings = [
        ...(data?.events ?? []).filter(event => event.status === 'prepared' || event.status === 'in-progress').map(event => ({ id: `event-${event.id}`, title: event.title, detail: event.playerBriefing || event.status })),
        ...(data?.pressures ?? []).filter(pressure => pressure.published && pressure.playerFacingUrgent).map(pressure => ({ id: `pressure-${pressure.id}`, title: pressure.title, detail: pressure.publicSummary || pressure.currentStatus })),
    ].slice(0, 6);
    return <main className="display atlas-page atlas-command-deck">
<style>{playerAtlasDeckCss}{inquisitorialBridgeCss}</style>
<aside className="atlas-ongoings" aria-labelledby="atlas-ongoings-title"><p className="eyebrow">Vox & astropathic traffic</p><h2 id="atlas-ongoings-title">Priority reports</h2><p className="atlas-rail-note">Time-sensitive developments the Cell has been cleared to know.</p>{ongoings.length ? ongoings.map(item => <article className="atlas-alert" key={item.id}><strong>{item.title}</strong><span>{item.detail}</span></article>) : <p className="atlas-empty">No priority reports are currently cleared for the Cell.</p>}</aside>
<header className="atlas-command-top"><div className="atlas-bridge-title"><span className="atlas-inquisition-sigil" aria-hidden="true">I</span><div><p className="eyebrow">Inquisitorial command bridge · public survey feed</p><h1>Tavrellis hololithic chart</h1></div></div><nav className="atlas-tool-strip" aria-label="Primary player tools"><a href="#/rosters">Army manager</a><a href="#/briefings">Briefings</a><a href="#/resources">Cell resources</a><a href="#/operation-briefing">Operations</a></nav></header>
<section className="atlas-system-panel" aria-label="Interactive Tavrellis system map"><div className="atlas-screen-heading"><span>Hololithic system survey · Tavrellis</span><span>Clearance: Cell public</span></div><div className="atlas-holomap"><div className="atlas-map"><img src="/hybrid-campaign/assets/world-atlas/tavrellis-system-unlabelled-map.png" alt="Tavrellis system map"/>{locations.filter(location => location.mapPosition).map(location => <button className={`atlas-pin ${location.mapPosition!.x > 79 ? 'atlas-pin-right-edge' : ''}`} key={location.id} aria-label={`View ${location.title}`} title={location.title} style={{ left: `${location.mapPosition!.x}%`, top: `${location.mapPosition!.y}%` }} onClick={() => setSelectedId(location.id)}><span className="atlas-pin-dot" aria-hidden="true"/><span className="atlas-pin-label" aria-hidden="true">{mapLabel(location.title)}</span></button>)}</div></div><article className="atlas-detail">{selected ? <><span className="world-meta">{selected.detailStatus === 'dossier' ? 'Dossier' : 'Map-only'} · {selected.publicationStatus}</span><h2>{selected.title}</h2><p>{selected.publicDescription}</p><p><strong>Access:</strong> {selected.authorityAccess}</p>{selected.consequences?.filter(entry => entry.published).map(entry => <p key={entry.at}><strong>Known consequence:</strong> {entry.detail}</p>)}</> : <p>Loading Tavrellis system entries…</p>}</article></section>
<aside className="atlas-quick" aria-labelledby="atlas-quick-title"><p className="eyebrow">Sanctioned command terminal</p><h2 id="atlas-quick-title">Requisition & records</h2><a href="#/exchange"><strong>Miren’s counter</strong><span>Trade Coin exchange and available objective credit.</span></a><a href="#/resources"><strong>Resources</strong><span>Approved shared and personal discoveries.</span></a><a href="#/briefings"><strong>Campaign archive</strong><span>Published recaps, leads, and directives.</span></a><a href="#/rosters"><strong>Force record</strong><span>Roster, readiness, and unit progression.</span></a><a href="#/operation-briefing"><strong>Current operation</strong><span>Assigned objectives and recovery information.</span></a></aside>
</main>;
}
